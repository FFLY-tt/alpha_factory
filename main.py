...
from quick_test import run_alpha_pipeline
...
def main():
    ...
    # 将弹药移交执行器 (纯算力计算)
    # =========================================================
    print(f"\n📦 将 {len(initial_parents)} 个父本移交执行器...")
    ...
    # 【外科手术修改】：把配置全量透传给流水线
    qualified_factors = run_alpha_pipeline(
        engine=engine,
        db=db,
        batch_id=current_batch_id,
        initial_beam=initial_parents,
        start_depth=start_depth,
        target_depth=TARGET_DEPTH if CURRENT_MODE == RunMode.XA else start_depth,
        POOL=POOL,
