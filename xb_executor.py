# xb_executor.py
"""
AlphaForge 执行器：XB 模式 (Step Forward)
逻辑：承接历史精英父本，向下深挖一级，并完成全流程评估与精调。
"""
import warnings
...
def run_xb_pipeline(
    ...
    t0 = time.time()
    print("=" * 70)
    print(f"⚙️ 启动 AlphaForge XB 递进流水线 (Target Depth: {target_depth}，换手上限{TURNOVER_CAP})")
    print("=" * 70)

    # ---------------------------------------------------------
    # Step 1：IS 样本内演化 (仅执行一层)
    # ---------------------------------------------------------
    print(f"\n🌱 [Step 1] 承接父本进行变异（IS: {IS_START_DATE} ~ {IS_END_DATE}）...")
    current_beam = initial_beam

    # 在 XB 模式下，start_depth 通常等于 target_depth (均为 RESUME_DEPTH + 1)
    for depth in range(start_depth, target_depth + 1):
        ...
        # 标记为 XB_MINING
        db.save_factor_batch(batch_id, "XB_MINING", depth, db_layer_input, variant_tag=f"depth_{depth}_xb_elite")

    # ---------------------------------------------------------
    # Step 2：A 组评估 (逻辑与 XA 完全对齐)
    # ---------------------------------------------------------
    print("\n" + "=" * 70)
    ...
    for base_name, info in best_is_versions.items():
        ...
        oos_ok = (
                ...
                (is_r['sharpe_net'] * oos_r['sharpe_net']) > 0
        )
